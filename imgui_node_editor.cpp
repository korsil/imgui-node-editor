...
void ed::EditorContext::UpdateNodeState(Node* node)
{
    ...
    node->m_Bounds.Min      = ImFloor(settings->m_Location);
    node->m_Bounds.Max      = ImFloor(node->m_Bounds.Min + settings->m_Size);
    node->m_GroupBounds.Min = ImFloor(settings->m_Location);
    node->m_GroupBounds.Max = ImFloor(node->m_GroupBounds.Min + settings->m_GroupSize);
}
